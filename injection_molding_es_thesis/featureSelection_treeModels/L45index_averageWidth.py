...
data = pd.read_csv(r".\data\new_L45_index_averageWidth.csv")

print(data.info())

# averageWidth 五數彙總、boxplot
dataaverageWidth = data['averageWidth'].describe()

print(dataaverageWidth)
averageWidthP25 = dataaverageWidth['25%']
averageWidthP75 = dataaverageWidth['75%']
print(averageWidthP75)
print(averageWidthP25)
IQR = averageWidthP75-averageWidthP25
print(IQR)
upper_boundary_limit_averageWidth = round(averageWidthP75 + 1.5*IQR, 4)
lower_boundary_limit_averageWidth = round(averageWidthP25 - 1.5*IQR, 4)
e_upper_boundary_limit_averageWidth = round(averageWidthP75 + 3*IQR, 4)
e_lower_boundary_limit_averageWidth = round(averageWidthP25 - 3*IQR, 4)
extrem_normal_interval = [e_lower_boundary_limit_averageWidth, e_upper_boundary_limit_averageWidth]
normal_interval = [lower_boundary_limit_averageWidth, upper_boundary_limit_averageWidth]
print(f'Extreme normal averageWidth interval : {extrem_normal_interval} ')
print(f'Normal averageWidth interval : {normal_interval} ')

# boxplot
plt.boxplot(data['averageWidth'],
...
plt.title('Average Width Boxplot')
# plt.xlabel('')
plt.ylabel('Average Width (mm)')
plt.show()
#
# outlier index
outlierIndex = []
for outlierI, outlierV in enumerate(data['averageWidth']):
...
x = drop_outliers.iloc[:, 1:30].values # 去掉 no 這欄
y = drop_outliers.iloc[:, 30].values 
...
plt.title('Average Width in Decision Tree-Feature importances', fontsize=14)
plt.show()
...
plt.title('Decision Tree-Average Width prediction (train)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend()
plt.show()
...
plt.title('Decision Tree-Average Width prediction (test)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend(loc='best')
plt.show()
...
plt.ylabel('Features', fontsize=14)
plt.yticks(fontsize=14)
plt.title('Average Width in Random Forest-Feature importances', fontsize=14)
plt.show()
...
plt.title('Random Forest-Average Width prediction (train)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend()
plt.show()
...
plt.title('Random Forest-Average Width prediction (test)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend()
plt.show()
...
plt.plot(x_axis, y_train, 'o', color='lightcoral', label='True', markersize=8)
plt.plot(x_axis, predicted_train, 'o', color='navy', label='Predict(train)', markersize=6)
plt.title('XGBoost-Average Width prediction (train)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend()
plt.show()
...
plt.plot(x_axis_test, y_test, 'o', color='lightcoral', label='True')
plt.plot(x_axis_test, predicted_test,'o', color='navy', label='Predict(test)')
plt.title('XGBoost-Average Width prediction (test)')
plt.xlabel('No')
plt.ylabel('Average Width (mm)')
plt.legend(loc='best')
plt.show()
...
plt.ylabel('Features', fontsize=14)
plt.yticks(fontsize=14)
plt.title('Average Width in XGBoost-Feature importances', fontsize=14)
plt.show()